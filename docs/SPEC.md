...
js/vanilla.js   — 共享的 1.12.2 移动、生存与战斗常量/公式           -> window.Vanilla
js/noise.js     — Java LCG、Improved Noise、单纯形噪声与 FBM       -> window.Noise
...
- 弓按住使用键蓄力，松开后消耗箭和弓耐久并生成带重力、方块碰撞和生物命中的箭实体。
  ...
  箭命中方块后保留为可拾取实体。主手或副手持盾时按住使用键进入格挡；服务端按视角与伤害来源
  判断是否命中盾面，格挡会降低击退并按原始伤害消耗实际盾牌耐久。斧的满蓄力攻击可暂时禁用盾牌。
...
- HUD 快捷栏使用 182×22 基础尺寸，生命和饥饿图标以 8 像素间距排列。切换手持物品后，
  ...
  生存背包提供四个装备槽、一个副手槽，并绘制包含已穿盔甲的可转向立体玩家预览。
...
- 背包支持左/右键拿取、`Shift+点击` 快移、数字键与悬停槽交换、双击收集同类物品，
  ...
  `F` 交换当前主手与副手，盾牌可通过拖放或 `Shift+点击` 装入副手槽；主手交互优先于副手盾牌。
...
- 联机默认且普通玩家固定为生存模式。生命、饥饿、经验、背包、装备、副手、出生点、死亡掉落和重生写入
...
- 背包档案带递增 `inventoryRevision`。主手、副手、丢弃、拾取、合成、交易、投掷鸡蛋、耐久消耗和进食均由服务端校验当前修订号、
  快捷栏与物品类型后原子更新；旧档案包只能降低饥饿值，不能伪造进食恢复，也不能覆盖已经确认的背包。
  切换快捷栏使用即时动作包广播，副手变更随档案提交立即广播。快照中的装备摘要至少包含物品 ID 与是否附魔；
  远端玩家的主手和副手方块模型使用立方体，剑、工具、弓、盾牌、食物和火把使用分类姿态，其他物品使用双面带厚度模型。
- 格挡使用独立于攻击动作的持续状态。开始和结束格挡通过即时动作包提交，并继续包含在 20Hz 状态快照中纠错；
  服务端校验主手或副手确实持盾，优先使用副手盾牌并仅损耗实际格挡盾牌的耐久。远端玩家必须平滑显示
  对应手臂抬盾、放盾、双手物品模型及 block-hit 动作。
...
- 战斗包含 0.5 秒受伤无敌帧、下落暴击和冲刺击退。伤害缩放、锋利加成、护甲韧性和保护附魔必须
  使用 `Vanilla` 的共享 1.12.2 公式。敌对生物追踪必须通过视线检测；服务端地面生物使用带全局
  tick 预算、单次节点上限、路径缓存和重寻路冷却的局部 A*。目标优先级依次覆盖逃跑、追逐、
  食物引诱、回家和游荡，路径节点必须拒绝悬崖、液体和不可占据空间。
